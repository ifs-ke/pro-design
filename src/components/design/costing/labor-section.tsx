...
const LaborItem = memo(({ control, index, remove }: LaborItemProps) => {
  ...
  const watchedUnits = watch(`labor.${index}.units`);
  const watchedRate = watch(`labor.${index}.rate`);

  const totalItemCost = (parseFloat(String(watchedUnits)) || 0) * (parseFloat(String(watchedRate)) || 0);
  ...
  return (
    <div className="p-4 border rounded-lg bg-background space-y-4 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        ...
                        <FormControl>
                            <RadioGroup onValueChange={field.onChange} defaultValue={field.value} className="flex items-center pt-2 gap-4">
                                <FormItem className="flex items-center space-x-2"><FormControl><RadioGroupItem value="hourly" id={`hourly-${field.id}`} /></FormControl><FormLabel htmlFor={`hourly-${field.id}`} className="font-normal text-sm">Hourly</FormLabel></FormItem>
                                <FormItem className="flex items-center space-x-2"><FormControl><RadioGroupItem value="daily" id={`daily-${field.id}`} /></FormControl><FormLabel htmlFor={`daily-${field.id}`} className="font-normal text-sm">Daily</FormLabel></FormItem>
                            </RadioGroup>
                        </FormControl>
                        <FormMessage />
                        </FormItem>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            <FormField control={control} name={`labor.${index}.units`} render={({ field }) => (<FormItem><FormLabel className="text-xs">{rateType === 'hourly' ? 'Hours' : 'Days'}</FormLabel><FormControl><Input type="number" placeholder="8" {...field} /></FormControl><FormMessage /></FormItem>)} />
...
const LaborTotalDisplay = () => {
  const { control } = useFormContext<FormValues>();
  const labor = useWatch({ control, name: 'labor' });
  const totalLaborCost = labor?.reduce((acc, item) => acc + ((item.units || 0) * (item.rate || 0)), 0) || 0;
  return <span className="font-semibold">{formatCurrency(totalLaborCost)}</span>;
};

const LaborSummaryCard = () => {
  const { control } = useFormContext<FormValues>();
  const labor = useWatch({ control, name: 'labor' });

  const summary = useMemo(() => {
    const totalVendors = labor?.length || 0;
    const totalHours = labor?.reduce((acc, item) => item.rateType === 'hourly' ? acc + (parseInt(String(item.units), 10) || 0) : acc, 0) || 0;
    const totalDays = labor?.reduce((acc, item) => item.rateType === 'daily' ? acc + (parseInt(String(item.units), 10) || 0) : acc, 0) || 0;
    const totalCost = labor?.reduce((acc, item) => acc + ((item.units || 0) * (item.rate || 0)), 0) || 0;
    return { totalVendors, totalHours, totalDays, totalCost };
  }, [labor]);
  ...
  return (
    <Card className="bg-muted/40 shadow-inner border-dashed">
      ...
      <CardContent className="text-sm space-y-4">
        ...
        <Separator/>
        <div className="flex justify-between items-center"><span className="text-muted-foreground font-bold">Total Cost</span><span className="font-bold text-lg text-primary">{formatCurrency(summary.totalCost)}</span></div>
      </CardContent>
    </Card>
  );
};
...
const LaborList = () => {
  ...
  const handleAddLabor = () => append({ vendor: "", units: 1, rate: 0, rateType: 'hourly' });
        ...
        <Button variant="outline" className="w-full justify-start items-center p-4 h-auto rounded-xl bg-card shadow-sm">
           <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col items-start">
              ...
              <span className="text-sm text-muted-foreground">Total Cost: <LaborTotalDisplay /></span>
            </div>
          </div>
        </Button>
      </PopoverTrigger>
