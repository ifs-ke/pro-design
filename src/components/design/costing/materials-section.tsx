...
const MaterialItem = memo(({ control, index, remove }: MaterialItemProps) => {
  ...
  const watchedCost = useWatch({ control, name: `materials.${index}.cost` });
  const watchedQuantity = useWatch({ control, name: `materials.${index}.quantity` });
  const watchedDescription = useWatch({ control, name: `materials.${index}.description` });

  const totalItemCost = (parseFloat(String(watchedCost)) || 0) * (parseInt(String(watchedQuantity), 10) || 0);
...
const MaterialTotalDisplay = () => {
  const { control } = useFormContext<FormValues>();
  const materials = useWatch({ control, name: 'materials' });
  const totalMaterialCost = materials?.reduce((acc, item) => acc + ((item.cost || 0) * (item.quantity || 0)), 0) || 0;
  return <span className="font-semibold text-base">{formatCurrency(totalMaterialCost)}</span>;
};
...
const MaterialSummaryCard = () => {
  ...
  const summary = useMemo(() => {
    ...
    const totalCost = materials?.reduce((acc, item) => acc + ((item.cost || 0) * (item.quantity || 0)), 0) || 0;
    ...
    <Card className="bg-muted/40 shadow-inner border-dashed">
      ...
      <CardContent className="text-sm space-y-4">
        <div className="flex justify-between items-center">
          <span className="text-muted-foreground flex items-center"><ListTree className="w-4 h-4 mr-2"/> Total Unique Items</span>
          <span className="font-semibold">{summary.totalItems}</span>
        </div>
        <Separator/>
        <div className="flex justify-between items-center">
          <span className="text-muted-foreground flex items-center"><Sigma className="w-4 h-4 mr-2"/> Total Quantity</span>
          <span className="font-semibold">{summary.totalQuantity}</span>
        </div>
        <Separator/>
        <div className="flex justify-between items-center">
          <span className="text-muted-foreground font-bold">Total Cost</span>
          <span className="font-bold text-lg text-primary">{formatCurrency(summary.totalCost)}</span>
        </div>
      </CardContent>
    </Card>
  );
        ...
        <Button variant="outline" className="w-full justify-start items-center p-4 h-auto rounded-xl bg-card shadow-sm">
           <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col items-start">
              ...
              <span className="text-sm text-muted-foreground">Total Cost: <MaterialTotalDisplay /></span>
            </div>
          </div>
        </Button>
      </PopoverTrigger>
